// One-shot migration: pulls richer content from the v1 site (`fightin-tenth-site`)
// into Sanity. Idempotent: re-running upserts via deterministic IDs.
//
// Run with:
//   node --env-file=.env.local scripts/migrate-from-v1.mjs
//
...
// ===== v1 content (extracted from fightin-tenth-site/lib/content.ts) =====

const ORDER_URL =
  'https://us.amazon.com/Fightin-Tenth-Cold-Desert-Storm-ebook/dp/B0GWW6P518';
const AUTHOR_EMAIL = 'makaturaorlando@gmail.com';

const EXCERPT_BODIES = {
  1: [
...
const PRESS_COVERAGE = [
  {
    ...
    url: 'https://pass-the-torch.praesidus.com/an-america-worth-saving/',
    order: 10,
  },
  {
    slug: 'air-space-forces-magazine',
  ...
  {
    slug: 'wesa-confluence',
    outlet: 'WESA 90.5 · The Confluence',
    date: '2026-02-21',
    headline: 'Cold-War aviation, in his own voice',
    description: 'A morning conversation with Michael Makatura ahead of the May release.',
    url: '#',
    order: 60,
  },
];
...
async function run() {
  ...
  // 1. Patch siteSettings
  tx = tx.patch('siteSettings', (p) =>
    p.set({ preorderUrl: ORDER_URL, contactEmail: AUTHOR_EMAIL }),
  );

  // 2. Excerpts: replace 1, 5, 15 with full bodies. Delete stale 14.
  ...
  tx = tx.createOrReplace({
    ...
    body: blocks(EXCERPT_BODIES[15]),
  });
  tx = tx.delete('excerpt-14');

  // 3. Blog post bodies
  for (const [slug, body] of Object.entries(BLOG_BODIES)) {
    tx = tx.patch(`blogPost-${slug}`, (p) => p.set({ body: blocks(body) }));
  }

  // 4. Press items
  for (const item of PRESS_COVERAGE) {
    tx = tx.createOrReplace({
      _id: `pressItem-${item.slug}`,
      _type: 'pressItem',
      outlet: item.outlet,
      date: item.date,
